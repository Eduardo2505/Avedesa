...
cursor.execute("SELECT idGrpAve,archivo FROM grpave where idusuario='resAdmin' limit 1")
...
if rows_count !=0:
	...
	try:
		...
		sql1 = "UPDATE grpave SET dropbox =1,urlDropbox='%s',idusuario='resAdmin2' WHERE idGrpAve = '%s'" % (urlDropbox,fname)
		cursor.execute(sql1)
		db.commit()
